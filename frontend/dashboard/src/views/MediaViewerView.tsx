    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Multimodal Media Asset Inspector</h1>
      ...
      {/* Tabs Bar */}
      <div className="flex items-center space-x-3 border-b border-slate-200 bg-white px-4 rounded-t-xl pt-2 border-t border-l border-r">
        <button
      ...
      {!isAnalyzing && !analysisResult && (
        ...
        >
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900">Click or drag a media file here to analyze</h3>
            <p className="text-xs text-slate-500 mt-1">Supports Images (.jpg, .png), Videos (.mp4, .mov), and Audio (.mp3, .wav, .m4a)</p>
          </div>
          <span className="px-3 py-1 text-[11px] font-semibold bg-slate-100 text-slate-700 rounded-full border border-slate-200">
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-1 shadow-sm">
              <span className="text-[10px] font-bold text-slate-500 uppercase">File Name</span>
              <p className="text-xs font-semibold text-slate-900 truncate">{analysisResult.fileName}</p>
            </div>
            <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-1 shadow-sm">
              <span className="text-[10px] font-bold text-slate-500 uppercase">File Size & Checksum</span>
              <p className="text-xs font-mono text-slate-700">{analysisResult.fileSize} • {analysisResult.sha256.slice(0, 10)}...</p>
            </div>
            <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-1 shadow-sm">
              <span className="text-[10px] font-bold text-slate-500 uppercase">Authenticity Verdict</span>
              <div className="flex items-center space-x-1.5">
                <ShieldCheck className="w-4 h-4 text-emerald-600" />
                <span className="text-xs font-bold text-emerald-700">{analysisResult.forensics.verdict}</span>
              </div>
            </div>
            <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-1 shadow-sm">
              <span className="text-[10px] font-bold text-slate-500 uppercase">Authenticity Confidence</span>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-bold text-slate-900">{(analysisResult.forensics.authenticityScore * 100).toFixed(0)}%</span>
              </div>
            </div>
          </div>
          ...
          {activeTab === 'image' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-4 shadow-sm">
                <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Uploaded Visual Asset</h3>
                <div className="h-80 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-center overflow-hidden relative">
                  {previewUrl ? (
                ...
                <p className="text-xs text-slate-600">{analysisResult.forensics.evidence}</p>
              </div>
              ...
              <div className="space-y-6">
                <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-3 shadow-sm">
                  ...
                  <div className="p-4 bg-slate-50 rounded-lg border border-slate-200 font-mono text-xs text-slate-800 leading-relaxed whitespace-pre-wrap">
          ...
          {activeTab === 'video' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-4 shadow-sm">
                <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Video Playback & Keyframes</h3>
                <div className="h-80 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-center overflow-hidden">
                  {previewUrl ? (
                    <video controls src={previewUrl} className="w-full h-full object-contain" />
                  ) : (
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      <VideoIcon className="w-12 h-12 mb-2 text-slate-400" />
                      <span className="text-xs">{analysisResult.fileName}</span>
                    </div>
                  )}
                </div>
