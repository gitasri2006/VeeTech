    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Executive Digest & Story Clusters</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-3">
          <h2 className="text-xs font-bold text-slate-600 uppercase tracking-wider">Generated Briefs ({briefs.length})</h2>
          {briefs.map((brief) => (
            <div
              key={brief.id}
            ...
            >
              ...
              <h3 className="text-xs font-bold text-slate-900 leading-snug">{brief.title}</h3>
              <div className="flex items-center space-x-2 text-[10px] text-emerald-600 font-mono">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 text-xs">
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <span className="text-slate-500 block mb-1">Unattributable Sentences Rejected:</span>
                <strong className="text-emerald-700 font-mono text-sm">{selectedBrief.unattributable_count} (0% Hallucination Rate)</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <span className="text-slate-500 block mb-1">Total Stories Synthesized:</span>
                <strong className="text-slate-900 font-mono text-sm">{selectedBrief.cluster_ids.length} Articles</strong>
              </div>
            </div>
          </div>
        )}
